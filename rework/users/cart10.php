...
$database = new CreateDb(dbname: "user_db", tablename: "cart_itemhs10");

if (isset($_POST['add'])) {
    $cart_id = $_POST['cartid_hs'];
    $stock_status = $database->getStockStatus($cart_id);

    if ($stock_status === 'in-stock') {
        if (isset($_SESSION['cart'])) {
            $item_array_id = array_column($_SESSION['cart'], "cartid_hs");
...
<body>
    <?php require_once('php/headergr10.php'); ?>
    <div class="container">
        <div class="row text-center py-5">
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                component($row['images'], $row['title_of_the_bookHS'], $row['prices'], $row['cartid_hs'], $row['author'], $row['stock_status']);
            }
            ?>
        </div>
    </div>
